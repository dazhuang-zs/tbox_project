...
- 你说"苹果很好吃"，向量可能是 [0.8, 0.3, 0.1...]
- 你说"苹果手机很贵"，向量可能是 [0.2, 0.9, 0.7...]
- 你说"香蕉很甜"，向量可能是 [0.1, 0.2, 0.9...]

"苹果"这个词，在第一个和第二个句子里距离近（都是水果/手机），在第三个句子里距离远（水果但不是苹果）。

**这就是语义距离**：意思越接近的东西，向量距离越近。
...
**向量数据库索引**：HNSW图 → O(log n) 找到语义最接近的邻居

```python
# HNSW索引的生活类比
# B+树索引像查字典——你知道词在哪里，直接翻到那一页
# HNSW索引像问路——你问第一个人，他告诉你往某个方向走，
# 你再问下一个，以此类推，最后找到目的地
```
