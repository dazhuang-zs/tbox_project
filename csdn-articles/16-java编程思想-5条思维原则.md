...
下次Code Review时，拿这张清单对照检查：

**组合大于继承**
- [ ] 你的Service有没有`extends BaseService`？→ 有就考虑改成组合
- [ ] 继承深度超过2层？→ 大概率该重构了
- [ ] 新功能是通过override父类方法实现的？→ 这是继承滥用的信号

**面向接口**
- [ ] 类里有没有`new XxxServiceImpl()`直接new具体实现？→ 改成依赖注入接口
- [ ] 新增一个支付/通知/存储方式需要改多少代码？→ 只加一个实现类才对
- [ ] 单元测试mock依赖需要PowerMock/字节码增强？→ 说明你的依赖没有面向接口

**不变性优先**
- [ ] Entity/DTO/VO有没有公开的setter？→ 不是每个字段都需要setter
- [ ] 方法有没有修改了入参对象的状态？→ 这叫副作用，是大忌
- [ ] 返回的List/Map是内部的引用还是防御性拷贝？→ 没有拷贝就是定时炸弹

**快速失败**
- [ ] 方法入口有没有校验参数？→ Objects.requireNonNull是最低要求
- [ ] 错误是在发生的地方抛出的，还是传了5层才崩？
- [ ] switch/if-else有没有覆盖所有可能的状态？→ 没有default就等着哪天炸

**显式优于隐式**
- [ ] 能不能不运行代码就看懂一个方法的执行流程？
- [ ] @Autowired字段注入查到几个？→ 全改成构造器注入
- [ ] @Transactional方法有没有this.xxx()的自我调用？→ AOP不生效的经典坑

**评分**：每条1分，中3条以上就该重构了。中5条以上——你老板知道你在写这样的代码吗？
