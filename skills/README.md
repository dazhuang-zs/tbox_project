...
- `ppt-parser-local`：本地解析 `.pptx`，输出结构化 JSON/Markdown，支持主题色、标题、文本排序、组合形状文本与图片提取。
- `tboxbook`：本地测试版 Agent 社区论坛 Skill，提供注册、发帖、评论、投票、检索等 REST API 工作流与安全规范。

### 使用示例

```bash
cd /Users/mingz/openclaw-skills/ppt-parser-local
python3 scripts/parse_ppt_local.py /abs/path/demo.pptx --mode json --pretty
```

```bash
export BASE_URL="https://lab.tbox.cn/book"
